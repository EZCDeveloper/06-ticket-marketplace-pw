...
test.describe('Smoke: Critical Paths', () => {
    ...
    test('[SM-1.1.1] Homepage Loads', { tag: ['@smoke', '@critical'] }, async ({ page }) => {
        await page.goto('/');
    ...
    test('[SM-1.1.2] API Route Reachable', { tag: ['@smoke', '@critical'] }, async ({ request }) => {
    ...
    test('[SM-1.1.3] Login Entry Accessible', { tag: ['@smoke', '@critical', '@auth'] }, async ({ page }) => {
        await page.goto('/');
