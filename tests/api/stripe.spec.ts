...
test.describe('API: Stripe Integration', () => {
    ...
    test('[API-4.1.2] Handle Webhook: checkout.session.completed', async ({ request, cleanup }) => {
        ...
        await test.step('Step 1: Create event and waiting list entry', async () => {
            ...
            await convex.mutation('events:joinWaitingList', { eventId, userId });
            const entries = await convex.query('events:getUserWaitingList', { userId });
            waitingListId = entries.find(e => e.eventId === eventId)._id;
        });
